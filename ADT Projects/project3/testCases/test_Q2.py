...
class TestBruteForce(unittest.TestCase):
    ...
    def test_points_on_the_same_line(self):
        ...
        expected = [Point(1, 1), Point(2, 2), Point(3, 3)]  # Convex hull should be the two outer points
        result = convex_hull(points)
        self.assertEqual(sorted(result, key=lambda p: (p.x, p.y)), sorted(expected, key=lambda p: (p.x, p.y)))

    def test_convex_hull_with_extra_points_inside(self):
    ...
    def test_convex_hull_with_duplicate_points(self):
